...
function zodToHtmlInputProps(
...
): React.InputHTMLAttributes<HTMLInputElement> & { myrequired: boolean } {
  ...
  if (!('checks' in typedSchema._def)) return { myrequired: false };

  const { checks } = typedSchema._def;
  const inputProps: React.InputHTMLAttributes<HTMLInputElement> & { myrequired: boolean } = {
    myrequired: true,
  };
...
function AutoFormInput({ label, isRequired, fieldConfigItem, fieldProps }: AutoFormInputComponentProps) {
  return (
    <FormItem>
      ...
      <FormControl>
        <Input type="text" {...fieldProps} myrequired={String(isRequired)} />
      </FormControl>
...
type AutoFormComboBoxInputComponentProps = {
  values?: [string, string][];
} & AutoFormInputComponentProps;

function AutoFormComboBox({ label, isRequired, field, fieldConfigItem, values }: AutoFormComboBoxInputComponentProps) {
  const [open, setOpen] = useState(false);

  const tmpValues: [string, string][] = values || [['1', 'VN']];

  function findItem(value: any) {
    return tmpValues.find((item) => item[0] === value);
  }
    ...
    <FormItem>
      ...
      <FormControl>
        <Popover open={open} onOpenChange={setOpen}>
          ...
          <PopoverContent className="w-full p-0 shadow">
            <Command>
              <CommandInput placeholder="Select an option" />
              <CommandEmpty>No option found.</CommandEmpty>
              <CommandGroup>
                {tmpValues.map(([value, label]) => (
                  <CommandItem
                    key={value}
                    onSelect={() => {
...
const INPUT_COMPONENTS = {
  ...
  // Custom
  // countrySelect: AutoFormCountrySelect,
  ...
  return (
    <Accordion type="multiple" className="space-y-5">
      {Object.keys(shape).map((name) => {
        ...
        const isRequired = zodInputProps.myrequired; // ?? fieldConfigItem.inputProps?.required ?? false;
...
function AutoForm<SchemaType extends ZodObjectOrWrapped>({
  ...
  return (
    <Form {...form}>
      ...
      >
        <AutoFormObject schema={objectFormSchema} form={form} fieldConfig={fieldConfig} valueCombox={valueCombox} />
